...
<div class="space-y-8 pb-16" x-data="widgetCustomizer({
    ...
    bubbleTitle: '{{ old('bubble_title', $settings->bubble_title ?? 'Chat with us') }}',
    greeting: '{{ old('greeting_message', $settings->greeting_message ?? 'Hello! 👋 How can I help you today?') }}',
    placeholder: '{{ old('placeholder_text', $settings->placeholder_text ?? 'Type a message...') }}',
    theme: '{{ old('theme', $settings->theme ?? 'dark') }}',
    position: '{{ old('position', $settings->position ?? 'bottom-right') }}',
    questions: {{ Js::from(old('suggested_questions', $settings->suggested_questions ?? ['What services do you offer?', 'How can I get started?'])) }},
    apiKey: '{{ $activeKey }}',
    cdnUrl: '{{ $cdnUrl }}'
})">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold tracking-tight">Website Chat Widget</h1>
            <p class="text-slate-400 mt-1 text-base">Embed your trained RAG AI assistant on any website with a single CDN script tag.</p>
        </div>
    ...
    <div class="glass p-6 rounded-2xl border border-white/10 shadow-xl space-y-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                    ...
                    <span>HTML Embed Code (CDN)</span>
                </h3>
                <p class="text-xs text-slate-400 mt-0.5">Paste this snippet before the closing <code class="text-indigo-300 font-mono">&lt;/body&gt;</code> tag on any website, WordPress, Shopify, Webflow, or custom app.</p>
            </div>
            
            <div class="flex items-center gap-3 shrink-0">
                <!-- API Key Selection -->
                <div class="flex items-center gap-2">
                    <label class="text-xs text-slate-400 font-semibold uppercase">API Key:</label>
        ...
        <div class="relative bg-slate-950/80 rounded-xl p-4 border border-white/5 font-mono text-xs text-emerald-400 overflow-x-auto select-all">
        ...
        <div class="lg:col-span-7 space-y-6">
            <form action="{{ route('widget.update') }}" method="POST" class="space-y-6">
                ...
                <div class="glass p-6 rounded-2xl border border-white/5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center justify-between p-4 bg-slate-900/60 rounded-xl border border-white/5">
                            <div>
                                <label class="text-xs font-bold uppercase text-white block">Widget Status</label>
                                <span class="text-[11px] text-slate-400">Show widget on external sites</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-slate-900/60 rounded-xl border border-white/5">
                            <div>
                                <label class="text-xs font-bold uppercase text-white block">Sound Chime</label>
                                <span class="text-[11px] text-slate-400">Play chime on incoming reply</span>
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-400 mb-1.5">Assistant / Bot Name</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-400 mb-1.5">Launcher Bubble Title</label>
                ...
                <div class="glass p-6 rounded-2xl border border-white/5 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Primary Brand Color</label>
                        <div class="flex items-center gap-3">
                            ...
                            <input type="text" x-model="primaryColor" class="w-32 bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white uppercase focus:outline-none focus:border-indigo-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Color Mode</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-400 mb-2">Screen Position</label>
                            <div class="grid grid-cols-2 gap-2">
                ...
                <div class="glass p-6 rounded-2xl border border-white/5 space-y-4">
                    <h3 class="text-base font-bold text-white flex items-center gap-2">
                        ...
                        <span>Greeting & Suggested Questions</span>
                    </h3>

                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-400 mb-1.5">Welcome Greeting Message</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-400 mb-1.5">Input Box Placeholder</label>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="text-xs font-bold uppercase text-slate-400">Starter Questions (Clickable Chips)</label>
                            <button type="button" @click="addQuestion()" class="text-xs text-indigo-400 hover:text-indigo-300 font-bold uppercase tracking-wider">+ Add Chip</button>
                        </div>
                        
                        <div class="space-y-2">
                            <template x-for="(q, idx) in questions" :key="idx">
                                <div class="flex items-center gap-2">
                                    <input type="text" :name="'suggested_questions[' + idx + ']'" x-model="questions[idx]" placeholder="e.g. What are your pricing plans?" class="flex-1 bg-slate-900 border border-white/10 rounded-xl px-4 py-2 text-xs text-white focus:outline-none focus:border-indigo-500">
                ...
                <div class="glass p-6 rounded-2xl border border-white/5 space-y-4">
                    <h3 class="text-base font-bold text-white flex items-center gap-2">
                        ...
                        <span>Security & Allowed Domains</span>
                    </h3>
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-400 mb-1.5">Authorized Domains (CORS Restriction)</label>
                        <input type="text" name="allowed_domains" value="{{ implode(', ', $settings->allowed_domains ?? ['*']) }}" placeholder="* or example.com, myapp.com" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500">
                        <span class="text-[11px] text-slate-500 mt-1 block">Use <code class="text-indigo-400">*</code> to allow all domains, or enter specific comma-separated domain names.</span>
                    </div>
                </div>
        ...
        <div class="lg:col-span-5 space-y-4">
            <div class="sticky top-24 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs text-slate-400">Updates live as you type</span>
                </div>
                ...
                <div class="w-full h-[640px] rounded-2xl overflow-hidden border border-white/10 shadow-2xl flex flex-col"
                    ...
                    <!-- Browser Top Bar -->
                    <div class="h-10 px-4 flex items-center gap-2 border-b"
                            ...
                            <div x-show="previewOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" class="w-full max-w-[340px] h-[480px] rounded-2xl shadow-2xl flex flex-col overflow-hidden mb-3 border pointer-events-auto"
                                    ...
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-[10px] opacity-80 flex items-center gap-1">
                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-300"></span> Active Now
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div class="flex-1 p-3.5 overflow-y-auto space-y-3 text-xs" :class="theme === 'dark' ? 'bg-[#080d1a]' : 'bg-slate-50'">
                                    ...
                                    <div class="flex flex-col items-start max-w-[85%]">
                                        ...
                                        <span class="text-[9px] text-slate-400 mt-1 ml-1">Just now</span>
                                    </div>
                                    ...
                                    <template x-for="(m, i) in mockMessages" :key="i">
                                        <div class="flex flex-col" :class="m.role === 'user' ? 'items-end' : 'items-start'">
                                            <div class="p-2.5 rounded-2xl max-w-[85%]" 
                                                 ...
                                                 x-text="m.text"></div>
                                            <span class="text-[9px] text-slate-400 mt-1 px-1">Just now</span>
                                        </div>
                                    </template>
                                </div>
        ...
        mockAsk(text) {
            ...
            setTimeout(() => {
                this.mockMessages.push({
                    role: 'bot',
                    text: 'This is an instant preview demonstration! When embedded on your live site, responses are generated in real-time from your uploaded Knowledge Base.'
                });
            }, 500);
        },
