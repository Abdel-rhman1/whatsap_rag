...
@section('title', __('hub.create_custom_role') ?? 'Create Custom Role')
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700" x-data="{
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-white">{{ __('hub.create_custom_role') ?? 'Create Custom Role' }}</h2>
                <p class="text-xs text-slate-400 mt-0.5">{{ __('hub.create_role_subtitle') ?? 'Define custom capabilities tailored specifically for your team structure.' }}</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('roles.store') }}" method="POST" class="space-y-8">
        ...
        <div class="glass rounded-3xl p-6 md:p-8 border border-white/10 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">
                        {{ __('hub.role_slug') ?? 'Role Identifier (Key)' }} <span class="text-rose-400">*</span>
                    </label>
                    <input 
                        type="text" 
                        name="slug" 
                        x-model="roleSlug" 
                        required 
                        placeholder="e.g. senior_support_agent" 
                        class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-indigo-300 font-mono focus:outline-none focus:border-indigo-500 transition-colors"
                    >
                </div>
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea 
                        name="description" 
                        rows="2" 
                        placeholder="Brief summary of duties and responsibilities assigned to this role..."
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-8 py-3 rounded-xl accent-gradient text-white text-sm font-bold shadow-lg shadow-indigo-500/20 hover:opacity-90 transition-all flex items-center gap-2">
                ...
                <span>{{ __('hub.save_role') ?? 'Save Role & Permissions' }}</span>
            </button>
        </div>
    </form>
</div>
